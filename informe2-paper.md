...
> 3. ~~Traducir a inglés y transcribir a LaTeX con la plantilla IEEE Access (6 págs, 2 columnas)~~ — **hecho**: versión en inglés en `paper_ieee_access/` (`access.tex` compila con pdfLaTeX; PDF de ejemplo en `access.pdf`). Quedan por completar los placeholders de autores/DOI/fechas y, si hace falta, ajustar el largo (hoy ~7–8 págs).
